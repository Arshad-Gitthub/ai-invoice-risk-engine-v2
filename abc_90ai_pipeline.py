...
def run_pipeline():
    ...
    print("\n  Layer 1 - NLP Email Classifier (TF-IDF + GBM)...")
    ...
    print(f"  [OK] DBSCAN: eps={ensemble.dbscan.eps}, min_samples={ensemble.dbscan.min_samples}")
    print(f"  [OK] MLP Neural Net: hidden_layers=(64,32,16)")
    ...
    print(f"  [OK] Meta-learner trained (RandomForest 200 trees + CalibratedCV)")

    print("\n  Layer 9 - Continuous Learning Engine...")
    learner = ContinuousLearner()
    print(f"  [OK] PSI drift threshold={learner.DRIFT_THRESHOLD} | EWMA alpha={learner.alpha}")
    ...
    for invoice in LIVE_INVOICES:
        ...
        print(f"\n  L1 NLP   [{bar(nlp_r['risk_score'])}] {nlp_r['risk_score']:.3f}  -> {nlp_r['label_name']} (conf={nlp_r['confidence']:.2f})")

        dup_r      = dedup.check(invoice)
        dedup_risk = 1.0 if dup_r["is_duplicate"] else dup_r["match_score"]
        print(f"  L2 DEDUP [{bar(dedup_risk)}] {dedup_risk:.3f}  -> {dup_r['match_type']}: {dup_r['detail'][:52]}")

        if dup_r["is_duplicate"]:
            print(f"\n  [REJECTED] Duplicate detected. Pipeline stops here.")
            results.append({"invoice_number": invoice["invoice_number"], "vendor": invoice["vendor_name"],
                "amount": invoice["invoice_amount"], "variance_pct": invoice["amount_variance_pct"],
                "final_risk": 1.0, "decision": "REJECTED_DUPLICATE", "top_drivers": "Duplicate", "confidence": "HIGH"})
            continue
        ...
        print(f"       -> Ensemble avg: {ens_avg:.3f}  (dominant: {ens_r['dominant_model']})")

        vend_r = vendor_profiler.score(invoice)
        print(f"  L6 VENDOR[{bar(vend_r['risk_score'])}] {vend_r['risk_score']:.3f}  -> Mahal dist={vend_r['mahal_dist']} | trust={vend_r['vendor_trust']}")

        po_r = po_learner.score(invoice)
        print(f"  L7 PO    [{bar(po_r['risk_score'])}] {po_r['risk_score']:.3f}  -> Actual={po_r['actual_variance']:.1f}% | Learned tol={po_r['learned_tolerance']:.1f}%")
        ...
        print(f"\n  L8 META  [{bar(final_risk)}] {final_risk:.3f}  -> {meta_r['meta_model']}")
        print(f"       -> Top drivers: {meta_r['top_drivers']}")

        risk_scores.append(final_risk)

        icons = {"AUTO_POST": "[AUTO]", "REVIEW": "[REVIEW]", "HOLD": "[HOLD]"}
        icon  = icons.get(final_decision, "[?]")
        print(f"\n  {icon} FINAL DECISION: {final_decision}  ({final_risk*100:.1f}% risk | {meta_r['confidence']} confidence)")
        ...
        results.append({
            ...
            "top_drivers":    meta_r["top_drivers"],
